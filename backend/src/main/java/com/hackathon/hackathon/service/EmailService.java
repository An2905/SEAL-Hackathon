...
public class EmailService {
  ...
  public boolean sendStaffAccountInvite(
      String toEmail, String fullName, String password, String role) {
    ...
    String html =
        "<h2>Xin chào "
            ...
            + "<p>Tài khoản <b>"
            + escapeHtml(role)
            + "</b> của bạn đã được tạo trên hệ thống SEAL Hackathon.</p>"
            + "<p><b>Email đăng nhập:</b> "
            + escapeHtml(toEmail)
            + "</p>"
            + "<p><b>Mật khẩu tạm:</b> "
            + escapeHtml(password)
  ...
  public boolean sendAnnouncement(String toEmail, String fullName, String title, String content) {
    String html =
        "<h2>Xin chào " + escapeHtml(fullName) + "</h2><p>" + escapeHtml(content) + "</p>";
