...
function CriteriaCard({ c, index, totalWeight }) {
  ...
  return (
    <div style={styles.card}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={styles.cardStats}>
            <span style={styles.chip}>
              ⚖️ Trọng số: <b>{c.weight}%</b>
            </span>
            <span style={styles.chip}>
              🎯 Tối đa: <b>{c.maxScore} điểm</b>
            </span>
          </div>
      ...
      {open && c.description && <div style={styles.desc}>{c.description}</div>}
    </div>
  )
}
...
export default function JudgeCriteriaView({ roundId, roundName }) {
  ...
  if (!roundId) return <div style={styles.empty}>Vui lòng chọn vòng thi để xem tiêu chí.</div>
        ...
        <>
          ...
          {data.totalWeight !== 100 && (
            <div style={styles.warningBox}>
              ⚠️ Tổng trọng số đang là <b>{data.totalWeight}%</b>, chưa đủ 100% — ban tổ chức có thể điều chỉnh thêm.
            </div>
          )}
          ...
          <div style={styles.list}>
            {data.criteria.map((c, i) => (
              <CriteriaCard key={c.criteriaId} c={c} index={i} totalWeight={data.totalWeight} />
            ))}
          </div>
          ...
          <div style={styles.footer}>
            <span>
              Tổng trọng số: <b>{data.totalWeight}%</b>
            </span>
            <span>
              Điểm tối đa tổng cộng: <b>{data.totalMaxScore} điểm</b>
            </span>
          </div>
        </>
      )}
