...
public class CheckInRepository {
    ...
    private static final String TEAM_QUERY = """
            SELECT tr.registration_id, tr.status AS registration_status, tr.registered_at,
                   ...
                   CASE WHEN ci.checkin_id IS NOT NULL AND ci.checked_in = 1 THEN 1 ELSE 0 END AS checked_in
        ...
        try (
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    boolean alreadyAdded = team.getMembers().stream()
    ...
    public CheckInTeamResponse applyTeamCheckIn(String eventId, String teamId, String staffUserId, boolean checked) {
    ...
    private void upsertCheckIn(
            ...
            String userId,
            String staffUserId,
            boolean checked) throws SQLException {
        String updateSql = """
                UPDATE check_ins
    ...
    private void syncRegistrationStatus(Connection conn, String eventId, String teamId) throws SQLException {
        int totalMembers = countTeamMembers(conn, teamId);
        int checkedMembers = countCheckedMembers(conn, eventId, teamId);
        String newStatus = (totalMembers > 0 && checkedMembers == totalMembers) ? "APPROVED" : "PENDING";
    ...
    private int countCheckedMembers(Connection conn, String eventId, String teamId) throws SQLException {
        String sql = """
                SELECT COUNT(*)
