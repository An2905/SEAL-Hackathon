...
public class CriteriaRepository {
  ...
  public String insertCriteria(
      String roundId, String criterionName, double weight, double maxScore, String description) {
  ...
  public boolean updateCriteria(
      String criteriaId, String criterionName, double weight, double maxScore, String description) {
    String sql =
  ...
  public Optional<EventCriterion> findCriteriaById(String criteriaId) {
    String sql = "SELECT " + SELECT_COLUMNS + "FROM event_criteria WHERE criteria_id = ?";
    try (Connection conn = dataSource.getConnection();
        PreparedStatement ps = conn.prepareStatement(sql)) {
      ps.setString(1, criteriaId);
      try (ResultSet rs = ps.executeQuery()) {
