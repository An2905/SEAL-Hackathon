...
public class StaffService {
  ...
  public String registerAccount(String authHeader, CreateStaffAccountRequest request) {
    ...
    String roleLabel =
        "EXPERT_EXTERNAL".equals(dbRole) ? "Chuyên gia (bên ngoài)" : "Chuyên gia (nội bộ)";
    boolean emailSent =
        emailService.sendStaffAccountInvite(email, fullName, rawPassword, roleLabel);
    if (!emailSent) {
  ...
  public AnnouncementResponse sendAnnouncementToAll(
      String authHeader, SendAllAnnouncementRequest request) {
    authService.validateRole(authHeader, "COORDINATOR");
    ...
    if (content.isEmpty()) throw new BadRequestException("Content cannot be empty.");
  ...
  public AnnouncementResponse sendAnnouncementToParticipants(
      String authHeader, SendParticipantAnnouncementRequest request) {
    authService.validateRole(authHeader, "COORDINATOR");
    ...
    if (eventId.isEmpty()) throw new BadRequestException("Event ID cannot be empty.");
    if (title.isEmpty()) throw new BadRequestException("Title cannot be empty.");
    if (content.isEmpty()) throw new BadRequestException("Content cannot be empty.");
    if (roles == null || roles.isEmpty()) throw new BadRequestException("Roles cannot be empty.");

    // MENTOR / JUDGE_INTERNAL = đối tượng nhận thông báo theo bảng phân công, không phải role tài
    // khoản
    List<String> validRoles =
        Arrays.asList("STUDENT_FPT", "STUDENT_EXTERNAL", "MENTOR", "JUDGE_INTERNAL", "JUDGE");
    for (String role : roles) {
      if (!validRoles.contains(role))
        throw new BadRequestException("Invalid role: " + role + ". Accepted: " + validRoles);
    }

    if (!eventRepository.existsById(eventId)) throw new BadRequestException("Event not found.");
    ...
    if (response == null) throw new BadRequestException("Failed to create announcement.");

    // Send emails per role and accumulate recipient count
    int totalRecipients = 0;
    for (String role : roles) {
      List<User> participants = announcementRepository.findEventParticipantsByRole(eventId, role);
