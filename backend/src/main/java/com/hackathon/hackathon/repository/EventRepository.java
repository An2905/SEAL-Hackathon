...
public class EventRepository {
  ...
  public List<Event> findAllByStatus(String statusFilter) {
    ...
    try {
      ...
      if (statusFilter == null || statusFilter.isEmpty() || "ALL".equals(statusFilter)) {
        sql =
            "SELECT event_id, title, description, start_date, end_date, status, created_at FROM"
                + " events";
  ...
  public Event findDetailHeader(String eventId) {
    String sql =
        "SELECT e.event_id, e.title, e.description, e.start_date, e.end_date, e.status,"
            + " e.created_at, COUNT(DISTINCT tr.team_id) AS total_teams, COUNT(DISTINCT CASE WHEN"
            + " tr.status = 'PENDING' THEN tr.team_id END) AS pending_teams, COUNT(DISTINCT"
            + " c.category_id) AS total_categories, COUNT(DISTINCT r.round_id) AS total_rounds,"
            + " COUNT(DISTINCT a.award_id) AS total_awards FROM events e LEFT JOIN categories c ON"
            + " e.event_id = c.event_id LEFT JOIN rounds r ON e.event_id = r.event_id LEFT JOIN"
            + " team_registrations tr ON e.event_id = tr.event_id LEFT JOIN awards a ON e.event_id"
            + " = a.event_id WHERE e.event_id = ? GROUP BY e.event_id, e.title, e.description,"
  ...
  public List<Round> findRoundsByEventId(String eventId) {
    ...
    String sql =
        "SELECT round_id, name, start_date, end_date, submission_deadline FROM rounds WHERE"
            + " event_id = ? ORDER BY round_order";
    try (Connection conn = dataSource.getConnection();
        PreparedStatement ps = conn.prepareStatement(sql)) {
      ps.setString(1, eventId);
      try (ResultSet rs = ps.executeQuery()) {
  ...
  public List<TeamRegistration> findTeamRegistrationsByEventId(String eventId) {
    ...
    String sql =
        "SELECT tr.registration_id, t.team_id, t.team_name, tr.status FROM team_registrations tr"
            + " JOIN teams t ON tr.team_id = t.team_id WHERE tr.event_id = ?";
    try (Connection conn = dataSource.getConnection();
        PreparedStatement ps = conn.prepareStatement(sql)) {
      ps.setString(1, eventId);
      try (ResultSet rs = ps.executeQuery()) {
