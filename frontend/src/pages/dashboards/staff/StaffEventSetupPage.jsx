...
import { createEventCategory, createEventRound } from '../../../api/staffEventSetup'
...
function CreateCategoryForm({ eventId, disabled }) {
  ...
  const submit = async (e) => {
    ...
    setMessage(null)
    if (!form.name.trim()) {
      setMessage({ text: 'Vui lòng nhập tên track', type: 'error' })
      return
    }
    setLoading(true)
    try {
      ...
      setMessage({
        text: `Đã tạo track "${created.name}" (#${created.categoryId})`,
        type: 'success'
      })
  ...
  return (
    <div className='card' id='setup-category'>
      <div className='card-head'>
        <div className='card-title'>Thêm track (category)</div>
      </div>
      <p className='card-sub'>
        Mỗi track là một hạng mục thi trong sự kiện (ví dụ: AI, Web, Mobile).
      </p>
...
function CreateRoundForm({ eventId, disabled }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setMessage({
        text: `Đã tạo vòng "${created.name}" (#${created.roundId}, thứ tự ${created.roundOrder})`,
        type: 'success'
      })
...
export default function StaffEventSetupPage() {
  const { showToast } = useToast()
  ...
  return (
    <>
      <div className='section-title'>
        <h2>Cấu hình track & vòng thi</h2>
        <span className='hint'>Chọn sự kiện rồi thêm category (track) hoặc vòng thi</span>
      </div>
      ...
      <div className='cards'>
        <div ref={categoryRef}>
          <CreateCategoryForm eventId={eventId} disabled={!ready} />
        </div>
        <div ref={roundRef}>
