...
function AssignJudgeForm({ judges, rounds, categories, disabled }) {
	...
	return (
		<div className="card">
			<div className="card-head"><div className="card-title">Phân công Giám khảo (theo vòng)</div></div>
			<p className="card-sub">Mỗi judge được gán vào một <strong>vòng</strong> trong một <strong>track</strong>.</p>
			<form className="form" onSubmit={submit}>
				...
				<FormField label="Vòng">
					<select name="roundId" value={form.roundId} onChange={handle} disabled={disabled || loading}>
						...
						{rounds.map((r) => (
							<option key={r.roundId} value={r.roundId}>{r.name} (#{r.roundId})</option>
						))}
					</select>
				</FormField>
				...
				<LoadingButton loading={loading} type="submit" disabled={disabled}>Phân công judge</LoadingButton>
...
function AssignMentorForm({ mentors, categories, disabled }) {
	...
	const [loading, setLoading] = useState(false);
	const [message, setMessage] = useState(null);
	const [form, setForm] = useState({ mentorId: "", categoryId: "" });
	const handle = (e) => setForm((f) => ({ ...f, [e.target.name]: e.target.value }));

	const submit = async (e) => {
		e.preventDefault();
		setMessage(null);
		if (!form.mentorId || !form.categoryId) {
	...
	return (
		<div className="card">
			<div className="card-head"><div className="card-title">Phân công Mentor (theo track)</div></div>
			<p className="card-sub">Mỗi mentor được gán vào một <strong>track</strong> của sự kiện.</p>
			<form className="form" onSubmit={submit}>
				<FormField label="Mentor">
					<select name="mentorId" value={form.mentorId} onChange={handle} disabled={disabled || loading}>
				...
				<LoadingButton loading={loading} type="submit" disabled={disabled}>Phân công mentor</LoadingButton>
				<FormMessage message={message?.text} type={message?.type} />
			</form>
		</div>
	);
...
export default function StaffAssignPage() {
	...
	useEffect(() => {
		...
		(async () => {
			try {
				const [ev, js, ms] = await Promise.all([
					getAllEvents("ALL"),
				...
				if (!cancelled) showToast(localizeError(err.message), "error");
			}
		})();
		return () => {
			cancelled = true;
	...
	return (
		<>
			...
			<div className="card">
				<div className="card-head"><div className="card-title">Chọn sự kiện</div></div>
				...
				{loadingDetail && <div className="empty-state" style={{ marginTop: 12 }}>Đang tải vòng & track…</div>}
				{ready && rounds.length === 0 && (
					<div className="empty-state" style={{ marginTop: 12 }}>Sự kiện này chưa có vòng nào.</div>
				)}
				{ready && categories.length === 0 && (
					<div className="empty-state" style={{ marginTop: 12 }}>Sự kiện này chưa có track nào.</div>
				)}
			</div>
